...
router.post(
  ...
  function (req, res, next) {
    try {
      pool.query(
        "insert into picture (picturetype,picture) values(?,?)",
        [req.body.picturetype, req.body.picture],
        function (error, result) {
          if (error) {
            console.log(error);
            res
            ...
            res
              .status(200)
              .json({ status: "true", msg: "picture Submited Succesfully" });
          }
        }
      );
    } catch (e) {
      ...
      res
        ...
        .json({
          status: false,
          msg: "Server Error: Pls Contact Server Admin.",
        });
    }
  }
);
